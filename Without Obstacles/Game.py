...
def Game(Neuron, Snake_game, SIZE, STEPS_PER_GAME, EPISODES, min_len4train, epsilon, epsilon_mul_value, TO_SHOW_GAME, UPDATE_SECONDARY_WEIGHTS_NUM):
    m=0
    score = []
    for epi in tqdm(range(1, EPISODES + 1), ascii=True, unit='Episodes'):
        Game = Snake_game(SIZE)
        Game.end = False
        count = 0
        while True:
            count += 1
